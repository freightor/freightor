...
(function (Bu, Bs, Ba, Bo)
{
   ...
   Bee.Dom = {
       ...
       * @param activateOnly {Boolean}
       * */
      setActive : function (newActiveEL, classDeactivate, activeClassName, activateOnly = false)
      {
         var pel = newActiveEL.parentNode; // returns single entity
         var pelChin = pel.children; //array(node list)
         var i = 0, len = pelChin.length
         if(!activateOnly)
         {
            for (; i < len; i++)
            {
            ...
            for (; i < len; i++)
            {
               pelChin[i].classList.remove(activeClassName);
            }
         }
      ...
      getTop : function (el)
      {
         return Bu.pInt(Bu.defined(el.style.top) && !Bs.isEmpty(el.style.top) ?
                        this.gsv('top', el) : el.offsetTop);
      },
       ...
       * @param el
       * @param globalHost
       * @param fullyInView {Boolean}
       * @returns {boolean}
       */
      isInVerticalView : function (el, globalHost = window, fullyInView)
      {
         let pageTop = globalHost.scrollTop;
         ...
         console.log(el.scrollTop, el.scrollHeight);
         console.log(this.getTop(el) + globalHost.scrollTop, 0, this.getBottom(el), window.innerHeight);
         return this.getTop(el) +  globalHost.scrollTop > 0 && this.getBottom(el) < window.innerHeight;
      },
